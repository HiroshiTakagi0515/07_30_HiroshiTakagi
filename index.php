...
// 実行したいSQL文（最新順番3つ取得） 投稿時間順に並べて新しいものから順に並べる
$sql ='SELECT * FROM gs_an_table ORDER BY time DESC LIMIT 3';

//MySQLで実行したいSQLセット。プリペアーステートメントで後から値は入れる
$stmt = $pdo->prepare($sql);
$flag = $stmt->execute();

//失敗した場合はエラーメッセージ表示して処理を止めるようになっている、成功した時はしたから2行目の}までの処理をする
if($flag==false){
...
<?php
				...
				$result.="<tr><th>お名前</th><th>EMAIL</th><th>年齢</th><th>性別</th><th>身長</th><?tr>";
?>


				<h3 class="work-title"><?php echo $result['book'];?></h3>
				<h3 class="work-title"><?php echo $result['internet'];?></h3>
				<h3 class="work-title"><?php echo $result['comment'];?></h3>
